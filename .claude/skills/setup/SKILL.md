...
After bootstrap succeeds, install the OneCLI CLI tool and start the OneCLI server.

**Apple Container (macOS):** Use the included script — no Docker or curl-pipe-sh needed:
```bash
bash start-onecli.sh
```
This starts a postgres container and the OneCLI app container using Apple Container, then prints the dashboard URL. Data is persisted in `~/.local/share/onecli/`.

**Docker:** Use the upstream quickstart:
```bash
git clone https://github.com/onecli/onecli.git /tmp/onecli
docker compose -f /tmp/onecli/docker/docker-compose.yml up -d
```

**CLI tool (both):** Install the `onecli` command-line client:
```bash
curl -fsSL onecli.sh/cli/install | sh
```

Verify: `onecli version`. If not found, add the printed install path to PATH.
...
**Service not starting:** Check `logs/nanoclaw.error.log`. Common: wrong Node path (re-run step 7), OneCLI not running (`bash start-onecli.sh` or `curl http://localhost:10254/api/health` to verify), missing channel credentials (re-invoke channel skill).
