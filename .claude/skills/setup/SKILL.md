...
Credentials are stored in `.env` and injected by the built-in credential proxy — containers never see raw secrets.
...
Verify the proxy starts: `npm run dev` should show "Credential proxy started" in the logs.
...
- macOS: `launchctl unload ~/Library/LaunchAgents/com.nanoclaw.v1.plist`
...
- SERVICE=stopped → `npm run build`, then restart: `launchctl kickstart -k gui/$(id -u)/com.nanoclaw.v1` (macOS) or `systemctl --user restart nanoclaw` (Linux) or `bash start-nanoclaw.sh` (WSL nohup)
- SERVICE=not_found → re-run step 7
- CREDENTIALS=missing → re-run step 4 (check `.env` has `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY`)
...
**Service not starting:** Check `logs/nanoclaw.error.log`. Common: wrong Node path (re-run step 7), missing credentials (check `.env` has `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY`, look for "Credential proxy started" in logs), missing channel credentials (re-invoke channel skill).
...
**Unload service:** macOS: `launchctl unload ~/Library/LaunchAgents/com.nanoclaw.v1.plist` | Linux: `systemctl --user stop nanoclaw`
