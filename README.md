...
- **Credential security** - Agents never hold raw API keys. Credentials are stored in `.env` on the host and injected at request time by the built-in credential proxy — containers never see secrets.
