...
export function markProcessing(ids: string[]): void {
  ...
  const stmt = db.prepare(
    "INSERT OR REPLACE INTO processing_ack (message_id, status, status_changed) VALUES (?, 'processing', strftime('%Y-%m-%dT%H:%M:%SZ', 'now'))",
  );
  db.transaction(() => {
...
export function markCompleted(ids: string[]): void {
  ...
  const db = getOutboundDb();
  const stmt = db.prepare(
    "INSERT OR REPLACE INTO processing_ack (message_id, status, status_changed) VALUES (?, 'completed', strftime('%Y-%m-%dT%H:%M:%SZ', 'now'))",
  );
  db.transaction(() => {
    for (const id of ids) stmt.run(id);
  })();
...
export function markFailed(id: string): void {
  getOutboundDb()
    .prepare(
      "INSERT OR REPLACE INTO processing_ack (message_id, status, status_changed) VALUES (?, 'failed', strftime('%Y-%m-%dT%H:%M:%SZ', 'now'))",
    )
    .run(id);
}
