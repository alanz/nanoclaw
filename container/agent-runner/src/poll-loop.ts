...
export interface PollLoopConfig {
  ...
  /**
   * Optional abort signal — when fired, the loop exits and any active query is
   * aborted. In production the loop runs until the container dies; tests pass a
   * signal so an abandoned loop actually exits instead of polling forever and
   * stealing messages from the next test's DB.
   */
  signal?: AbortSignal;
}
