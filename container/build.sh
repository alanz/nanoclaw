...
# Apple Container needs its builder VM up before any `container build`. Only on
# the paths that actually build — the pull path never invokes the builder.
if [ "$CONTAINER_RUNTIME" = "container" ] && [ "$PULL" != "true" ]; then
...
if [ "$PULL" = "true" ]; then
    ...
    if [ "$CONTAINER_RUNTIME" = "container" ]; then
        # Don't drop the builder-stop trap set above.
