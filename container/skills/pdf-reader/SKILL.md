---
name: pdf-reader
description: Read and extract text from PDF files — documents, reports, contracts, spreadsheets. Use whenever you need to read PDF content, not just when explicitly asked. Handles local files, URLs, and DeltaChat attachments.
...
## DeltaChat PDF attachments

When a user sends a PDF via DeltaChat, it is automatically saved to the `attachments/` directory. The message will include a path hint like:

> [File: /workspace/ipc/attachments/123-document.pdf]

To read the attached PDF:

```bash
pdf-reader extract /workspace/ipc/attachments/123-document.pdf --layout
```
...
```bash
pdf-reader info /workspace/ipc/attachments/123-contract.pdf
pdf-reader extract /workspace/ipc/attachments/123-contract.pdf --layout
```
