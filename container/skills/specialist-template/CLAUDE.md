...
If your task also produced other files (PDFs, images, data, code), include those in `file_paths` too:

```
mcp__nanoclaw__deliver_specialist_result(
  result_text="Summary of findings...",
  file_paths=["/workspace/ipc-out/result.md", "/workspace/ipc-out/data.csv"]
)
```
