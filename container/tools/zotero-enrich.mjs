 ...
 *   node /app/tools/zotero-enrich.mjs --dir /workspace/agent/zotero-md
 *   node /app/tools/zotero-enrich.mjs --dir /workspace/agent/zotero-md --dry-run
 *   node /app/tools/zotero-enrich.mjs --dir /workspace/agent/zotero-md --limit 10
 *
...
async function findAbstract(meta, title) {
  ...
  if (doi && !SKIP_S2) {
    ...
    if (ab) return [ab, 's2_doi'];
  }

  // 2. DOI → CrossRef
  if (doi && !SKIP_CR) {
    const ab = await crByDoi(doi);
    if (ab) return [ab, 'cr_doi'];
  }
  ...
  if (urlDoi && urlDoi !== doi) {
    if (!SKIP_S2) {
      ...
      if (ab) return [ab, 's2_url_doi'];
    }
    if (!SKIP_CR) {
      const ab = await crByDoi(urlDoi);
      if (ab) return [ab, 'cr_url_doi'];
    }
  }
