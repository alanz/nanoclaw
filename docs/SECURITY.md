...
Real API credentials **never enter containers**. NanoClaw uses a built-in credential proxy (`src/credential-proxy.ts`) to intercept outbound requests and inject credentials at the network level.

**How it works:**
1. Credentials are stored in `.env` on the host, never passed to containers
2. When NanoClaw spawns a container, it sets `ANTHROPIC_BASE_URL` to point at the credential proxy
3. The proxy intercepts API requests, injects the real credential, and forwards to the upstream API
