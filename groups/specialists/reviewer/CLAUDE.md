...
mcp__nanoclaw__deliver_specialist_result(
  result_text="One-sentence review summary.",
  file_paths=["/workspace/ipc-out/result.md"]
)
```

If your task also produced other files (annotated diffs, reports), include those in `file_paths` too:

```
mcp__nanoclaw__deliver_specialist_result(
  result_text="Review summary...",
  file_paths=["/workspace/ipc-out/result.md", "/workspace/ipc-out/review.diff"]
)
```
