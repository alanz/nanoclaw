 ...
 */
function imageDigest(image: string, expectRepo?: string, runCmd = 'docker'): string {
  const res = spawnSync(
    runCmd,
...
export async function run(args: string[]): Promise<void> {
  ...
  // Runtime readiness. Fork keeps two runtimes; upstream's step is Docker-only.
  if (runtime === 'apple-container') {
    if (!appleContainerRunning() && !(await tryStartAppleContainer())) {
      emitStatus('SETUP_CONTAINER', {
  ...
  // Build — stdio inherit so the parent setup runner can tail the runtime's
  ...
  if (source === 'pull') {
    ...
    if (pullRes.status === 0) {
      ...
      // own derived image — `container-runner.ts` prefers that pin — so
