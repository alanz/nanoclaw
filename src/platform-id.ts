 ...
 * for DMs and 'group:<id>' for group chats. DeltaChat emits 'dc:<chatId>'
 * (NOT 'deltachat:<chatId>'). Prefixing any of these would cause a mismatch
 * with what the adapter later emits.
 */
export function namespacedPlatformId(channel: string, raw: string): string {
  ...
  // DeltaChat native adapter uses 'dc:<chatId>' — not 'deltachat:<chatId>'
  if (raw.startsWith('dc:')) return raw;
  return `${channel}:${raw}`;
}
